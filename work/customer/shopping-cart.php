...
if(isset($_POST['submit'])){
		...
		foreach($_POST['product_quantity'] as $key => $val){
			if($val==0){
				unset($_SESSION['cart'][$key]);
			}else{
				$_SESSION['cart'][$key]['product_quantity']=$val;
 ...
 $pdtid=array();
    $sql = "SELECT * FROM product_details WHERE product_id IN(";
			...
			while($row = mysqli_fetch_array($query)){
				$quantity=$_SESSION['cart'][$row['product_id']]['product_quantity'];
				$subtotal= $_SESSION['cart'][$row['product_id']]['product_quantity']*$row['productPrice']+$row['shippingCharge'];
				$totalprice += $subtotal;
